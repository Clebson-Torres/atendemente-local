...
pub async fn create_backup(
    ...
    let file_name = format!("backup_{}.zip", created_at.format("%Y%m%d_%H%M%S"));
    ...
    if storage_root.exists() {
        collect_files(&storage_root, &storage_root, &mut entries).await?;
    }
    ...
    audit::write_audit_event(
        ...
        "backup",
        None,
        serde_json::json!({"file_name": file_name, "entries": manifest.file_hashes.len()}),
        Some("local-device"),
    )
    .await?;
...
pub async fn restore_backup(
    ...
    let mut archive = zip::ZipArchive::new(Cursor::new(backup_bytes))
        .map_err(|_| AppError::bad_request("Backup invalido ou corrompido."))?;
    let manifest = read_manifest(&mut archive)?;
    if manifest.version != BACKUP_VERSION {
        return Err(AppError::bad_request("Versao de backup nao suportada."));
    }
    ...
    audit::write_audit_event(
        ...
        "backup",
        None,
        serde_json::json!({"version": manifest.version, "entries": manifest.file_hashes.len()}),
        Some("local-device"),
    )
    .await?;
